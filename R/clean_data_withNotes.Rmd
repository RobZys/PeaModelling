...
LN <- df_lincoln %>% 
  as_tibble(.name_repair = "universal") %>% 
  mutate(Index = "LN") %>%
  mutate_at(vars(starts_with("N_Content")), as.double)
...
2. is final grain count counts the sprouted ones? -- assume that it does --- need to confirm with Mike.
    a. the final grain count does not have the sprouted ones. 
    b. final grain fresh weight is separated for sprouted and normal grain based on the excel sheet - need to confirm with Nathan. 
...
whole_plant <- combined %>% 
  ...
  mutate(# subsample fresh/dry weight 
         ...
         #pod
         greenpodFW = ifelse(greenpodFW ==0, Final_podFW, greenpodFW), 
         greenpodDW = ifelse(greenpodDW ==0, Final_podDW, greenpodDW), 
         maturepodFW = ifelse(maturepodFW == 0, Final_podFW, maturepodFW  ),
         maturepodDW = ifelse(maturepodDW == 0, Final_podDW, maturepodDW  ),
         ...
         maturegrainFW = ifelse(maturegrainFW == 0, Final_grainFW, maturegrainDW),
         maturegrainDW = ifelse(maturegrainDW == 0, Final_grainDW, maturegrainDW),
...
whole_plant %>% 
  mutate(partDM_PC = partDW/partFW,
         SSDM_PC = SSDW/ SSFW) %>% 
  filter(Plot %in% c(1, 5,11),
         Site == "HB") %>% 
  select(Plot, Site, Date, Event, SSFW, SSDW, partFW, partDW, partDM_PC, SSDM_PC) %>% 
  filter(partDM_PC > 0.7)
```
...
```{r}
LN_mike <- scrape_xl(url = url, sheet = "CalculatedDataLincoln", skip = 2)
LN_mike_yield <- LN_mike %>% 
  select(Site = Index, Date, Plot, Trt_num, Trt_name, Cultivar, Sowing_Date, N_timing, ends_with("Yield"), starts_with("Fi_")) %>% 
  mutate(Site = "LN",
         Date = as.Date(Date),
         TotalDryYield = ifelse(is.na(TotalDryYield), Fi_TotalYield, TotalDryYield))
```
...
# dbWriteTable(con, name = "LN_mike_yield", value = LN_mike_yield, overwrite=TRUE)
# 
...
```{r example of converting the date}
con <- dbConnect(RSQLite::SQLite(),'../Data/Pea_data.sqlite3')
dbListTables(con)
